...
- 如果用户查询天气，即使说“搜索天气”，也优先选择 get_weather，因为用户目标是天气数据。
- 如果用户查询当前位置，选择 get_location。
- 如果工具参数不完整但工具支持自动补全，可以传空字符串。例如 get_weather 的 city 可为空，会自动使用 IP 定位城市。
...
返回格式必须是：
{"action": "tool", "tool_name": "get_weather", "args": {"city": "长沙"}, "confidence": 0.95, "reason": "一句简短中文原因"}

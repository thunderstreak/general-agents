...
`agent_app/file_inputs/parser.py` 支持 `@filepath` 语法，解析 txt/md/json/csv/pdf/docx/xlsx/图片（base64 多模态）。

### LLM 管理

`agent_app/llm.py` 用 `@lru_cache` 按用途管理多个 `ChatOpenAI` 实例（chat、tool_selector、vision 等），均通过环境变量配置。

## 配置

所有配置在 `.env` 中（参考 `.env.example`），由 `agent_app/config.py` 加载。关键项：各用途模型名、API Base URL、超时、记忆文件路径、编排最大步数。

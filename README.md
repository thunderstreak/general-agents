...
├── .env.example                     # 环境变量示例
├── agent_app/
│   ├── cli/                         # CLI 交互、流式输出、取消、会话/RAG/compact/memory 命令
│   ├── config.py                    # 环境变量和运行配置
...
长期记忆文件使用 v2 schema，包含 `schema_version`、`summary` 和带稳定 ID 的 `items`。每条记忆的 ID 使用 `mem_` 前缀，可通过 `/memory list` 查看，并用于删除指定记忆。
...
`/memory list` 会显示 `memory_id=...`、分类、创建时间和内容。删除时复制 `memory_id` 后面的值，例如：
...
- `.agent_memory.json` 保存长期记忆摘要和用户偏好，长期记忆条目可通过 `/memory` 命令查看、删除和清空。
...
- `agent_app/cli/` 负责 CLI 主循环、输入读取、流式 chunk 解析、Esc 取消、会话命令、RAG 命令、长期记忆命令和上下文压缩命令。
